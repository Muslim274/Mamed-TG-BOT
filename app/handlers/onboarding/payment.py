...
message_links = {}
getcourse_pending = {}
# Хранилище состояния чекбоксов для обязательного ознакомления с документами
user_document_checkboxes = {}
...
def get_integrated_payment_keyboard(payment_url: str = None):
    ...
    keyboard_buttons.extend([
        ...
        [
            InlineKeyboardButton(
                text="💎 Что я приобретаю",
                callback_data="what_i_get"
            )
        ],
        [
            InlineKeyboardButton(
        ...
        [
            InlineKeyboardButton(
                text="🌍 Иностранные карты / СНГ",
                callback_data="show_foreign_cards_payment"
            )
        ],
        [
            InlineKeyboardButton(
                text="❓ Проблемы с оплатой / Техподдержка",
...
async def intro_continue(callback: types.CallbackQuery, onboarding_user):
    logger.info("User %s continuing after intro - showing document review", callback.from_user.id)

    try:
        # Инициализируем чекбоксы для пользователя
        user_id = callback.from_user.id
        user_document_checkboxes[user_id] = {
            "doc1": False,
            "doc2": False,
            "doc3": False,
            "doc4": False
        ...
        # НЕ обновляем стадию здесь! Обновим только после подтверждения документов
        # Это предотвратит автоматическую отправку круглых видео

        # Отправляем сообщение о необходимости ознакомления
        intro_text = """
📄 <b>Обязательное ознакомление с документами</b>

Перед оплатой необходимо ознакомиться с важными документами.

📋 Сейчас вам будут отправлены 4 документа. Пожалуйста, ознакомьтесь с каждым из них.

После ознакомления подтвердите это, нажав на соответствующие кнопки ниже.

✅ Ссылка на оплату станет доступна только после подтверждения ознакомления со всеми документами.
"""

        await callback.message.answer(intro_text, parse_mode="HTML")

        # Отправляем 4 документа используя file_id из settings
        # Документ 1: Оферта
        try:
            await callback.message.answer_document(
                document=settings.DOC_OFERTA_FILE_ID,
                caption="📄 <b>Документ 1:</b> Оферта",
                parse_mode="HTML"
            )
            logger.info(f"✅ Document 1 (Оферта) sent successfully using file_id")
        except Exception as e:
            logger.error(f"❌ Error sending document 1 (Оферта): {e}", exc_info=True)
            await callback.message.answer("📄 <b>Документ 1:</b> Оферта\n<i>(Ошибка отправки)</i>", parse_mode="HTML")

        # Документ 2: Политика конфиденциальности
        try:
            await callback.message.answer_document(
                document=settings.DOC_PRIVACY_FILE_ID,
                caption="📄 <b>Документ 2:</b> Политика конфиденциальности",
                parse_mode="HTML"
            )
            logger.info(f"✅ Document 2 (Политика конфиденциальности) sent successfully using file_id")
        except Exception as e:
            logger.error(f"❌ Error sending document 2: {e}", exc_info=True)
            await callback.message.answer("📄 <b>Документ 2:</b> Политика конфиденциальности\n<i>(Ошибка отправки)</i>", parse_mode="HTML")

        # Документ 3: Пользовательское соглашение
        try:
            await callback.message.answer_document(
                document=settings.DOC_USER_AGREEMENT_FILE_ID,
                caption="📄 <b>Документ 3:</b> Пользовательское соглашение",
                parse_mode="HTML"
            )
            logger.info(f"✅ Document 3 (Пользовательское соглашение) sent successfully using file_id")
        except Exception as e:
            logger.error(f"❌ Error sending document 3: {e}", exc_info=True)
            await callback.message.answer("📄 <b>Документ 3:</b> Пользовательское соглашение\n<i>(Ошибка отправки)</i>", parse_mode="HTML")

        # Документ 4: Согласие на обработку персональных данных
        try:
            await callback.message.answer_document(
                document=settings.DOC_PERSONAL_DATA_FILE_ID,
                caption="📄 <b>Документ 4:</b> Согласие на обработку персональных данных",
                parse_mode="HTML"
            )
            logger.info(f"✅ Document 4 (Согласие на обработку персональных данных) sent successfully using file_id")
        except Exception as e:
            logger.error(f"❌ Error sending document 4: {e}", exc_info=True)
            await callback.message.answer("📄 <b>Документ 4:</b> Согласие на обработку персональных данных\n<i>(Ошибка отправки)</i>", parse_mode="HTML")

        # Отправляем клавиатуру с чекбоксами
        await send_document_checkboxes(callback.message, user_id)
        await callback.answer("📄 Ознакомьтесь с документами")
...
async def send_document_checkboxes(message, user_id: int):
    """Отправка клавиатуры с чекбоксами для подтверждения ознакомления"""
    checkboxes = user_document_checkboxes.get(user_id, {})

    checkbox_text = """
✅ <b>Подтвердите ознакомление с документами:</b>

Нажмите на каждую кнопку ниже, чтобы подтвердить ознакомление с соответствующим документом.
"""

    keyboard = get_document_checkboxes_keyboard(user_id)
...
def get_document_checkboxes_keyboard(user_id: int) -> InlineKeyboardMarkup:
    """Создание клавиатуры с чекбоксами"""
    checkboxes = user_document_checkboxes.get(user_id, {})

    def get_checkbox_text(doc_num: str, checked: bool) -> str:
        checkbox = "✅" if checked else "☑️"
        return f"{checkbox} Я ознакомился с Договором №{doc_num}"

    buttons = [
        [InlineKeyboardButton(
            text=get_checkbox_text("1", checkboxes.get("doc1", False)),
            callback_data="toggle_doc:doc1"
        )],
        [InlineKeyboardButton(
            text=get_checkbox_text("2", checkboxes.get("doc2", False)),
            callback_data="toggle_doc:doc2"
        )],
        [InlineKeyboardButton(
            text=get_checkbox_text("3", checkboxes.get("doc3", False)),
            callback_data="toggle_doc:doc3"
        )],
        [InlineKeyboardButton(
            text=get_checkbox_text("4", checkboxes.get("doc4", False)),
            callback_data="toggle_doc:doc4"
        )]
    ]
    ...
    if all_checked:
        buttons.append([
            InlineKeyboardButton(
                ...
                callback_data="documents_confirmed"
            )
        ])
...
@router.callback_query(F.data.startswith("toggle_doc:"))
async def toggle_document_checkbox(callback: types.CallbackQuery):
    """Переключение чекбокса документа"""
    user_id = callback.from_user.id
    doc_key = callback.data.split(":")[1]

    if user_id not in user_document_checkboxes:
        user_document_checkboxes[user_id] = {
            "doc1": False,
            "doc2": False,
            "doc3": False,
    ...
    user_document_checkboxes[user_id][doc_key] = not user_document_checkboxes[user_id][doc_key]
    ...
    try:
        ...
        await callback.answer("✅ Отмечено" if user_document_checkboxes[user_id][doc_key] else "☑️ Снято")
    except Exception as e:
        logger.error(f"Error toggling checkbox: {e}")
        await callback.answer()


@router.callback_query(F.data == "documents_confirmed")
async def documents_confirmed(callback: types.CallbackQuery):
    """Пользователь подтвердил ознакомление со всеми документами"""
    user_id = callback.from_user.id

    # Проверяем, что все чекбоксы отмечены
    checkboxes = user_document_checkboxes.get(user_id, {})
    if not all(checkboxes.values()):
        await callback.answer("❌ Пожалуйста, подтвердите ознакомление со всеми документами", show_alert=True)
        return

    logger.info(f"User {user_id} confirmed all documents - showing payment options")

    # Обновляем стадию пользователя до WAIT_PAYMENT ТОЛЬКО ПОСЛЕ подтверждения документов
    async with AsyncSessionLocal() as session:
        await StageUpdateHelper.update_user_stage(session, user_id, OnboardingStage.WAIT_PAYMENT, callback.bot)
        await session.commit()
        logger.info(f"✅ Пользователь {user_id} переведен в стадию WAIT_PAYMENT после подтверждения документов")
...
"""
    ...
    try:
        ...
        await callback.answer("✅ Документы подтверждены!")

        # Очищаем чекбоксы
        if user_id in user_document_checkboxes:
            del user_document_checkboxes[user_id]

    except Exception as e:
        logger.error("Error in documents_confirmed: %s", e, exc_info=True)
        await callback.answer("❌ Произошла ошибка", show_alert=True)


@router.callback_query(F.data == "show_foreign_cards_payment")
async def show_foreign_cards_payment(callback: types.CallbackQuery):
    """Показать реквизиты для оплаты иностранными картами / СНГ"""
    user_id = callback.from_user.id
    logger.info(f"User {user_id} opened foreign cards payment section")

    price = settings.ONBOARDING_COURSE_PRICE
    formatted_amount = f"{price:,.0f}".replace(",", "\u202F") + f" {settings.ONBOARDING_COURSE_CURRENCY}"

    foreign_cards_text = f"""
🌍 <b>Оплата через Иностранные карты / СНГ</b>
...
🇹🇷 <b>ТУРЦИЯ (IBAN):</b>
<code>TR620020500009732547800001</code>
👤 Получатель: Mamed Abdullaev

🇦🇿 <b>АЗЕРБАЙДЖАН (Номер карты):</b>
<code>4127214124826329</code>
👤 Получатель: JEYHUN ABDULLAYEV

🇰🇬 <b>КЫРГЫЗСТАН (Bakai Bank):</b>
<code>9417548506493580</code>
👤 Получатель: Рамазан Сейтов

📞 <b>СНГ / ОБЩИЙ (Номер телефона для оплаты):</b>
<code>+79003268720</code>
💡 <i>Этот номер также является платежным реквизитом</i>

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

<i>⬆️ Нажмите на реквизиты, чтобы скопировать ⬆️</i>

После оплаты нажмите кнопку "✅ Я оплатил(а)" для подтверждения.
"""
    ...
    try:
        ...
        await callback.answer("🌍 Реквизиты для оплаты")
    except Exception as e:
        logger.error(f"Error showing foreign cards payment: {e}")
...
async def back_to_payment_methods(callback: types.CallbackQuery):
    ...
    user_id = callback.from_user.id
    logger.info(f"User {user_id} returning to payment methods")
...
"""
    ...
    try:
        ...
        # Используем edit_text вместо answer, чтобы не создавать новое сообщение
        try:
        ...
        await callback.answer("🔙 Возврат к способам оплаты")

    except Exception as e:
        logger.error(f"Error in back_to_payment_methods: {e}")
        await callback.answer("❌ Ошибка", show_alert=True)
...
async def completed_steps(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Уведомление в техподдержку (с обработкой ошибок)
        try:
...
"""
            ...
            logger.info(f"✅ Support notification sent for user {user_id}")
        except Exception as support_error:
            logger.error(f"⚠️ Failed to send support notification: {support_error}")
...
async def what_i_get_handler(callback: types.CallbackQuery):
    """Обработчик кнопки 'Что я приобретаю'"""
    logger.info(f"User {callback.from_user.id} clicked 'Что я приобретаю' button")

    try:
        # Текст сообщения
        message_text = """
💎 <b>Что я приобретаю</b>

Зарабатывайте онлайн, не выходя из дома, получая 5️⃣0️⃣% с каждой продажи уроков от Мамеда — блогера, который говорит о саморазвитии, духовных ценностях и мотивации📌

🗣Работа ведётся прямо со смартфона:
вы выкладываете готовые видео в Instagram или YouTube, прикрепляете свою личную ссылку🔗 — и зарабатываете 2350 ₽ с каждой продажи.

🗣Вы получаете доступ к каналу 
MAMED PARTNERS
 , где присутствует лично Мамед:
он даёт рекомендации, отвечает на вопросы, выходит в эфиры и помогает вам расти

💬Также вы получаете сопровождение команды:
проверку ваших аккаунтов, подсказки  по оформлению .✔️

Этот заработок полностью в рамках шариата☪️:
никаких сомнительных действий, ставок, обмана или запретных элементов — только честная рекомендация полезного образовательного продукта.
"""
        ...
        # Отправляем скриншоты используя file_id из settings
        screenshot_file_ids = [
            settings.SCREENSHOT_1_FILE_ID,
            settings.SCREENSHOT_2_FILE_ID,
            settings.SCREENSHOT_3_FILE_ID,
            settings.SCREENSHOT_4_FILE_ID
        ]

        for i, file_id in enumerate(screenshot_file_ids, 1):
            try:
                await callback.message.answer_photo(photo=file_id)
                logger.info(f"✅ Screenshot {i} sent to user {callback.from_user.id} using file_id")
            except Exception as e:
                logger.error(f"❌ Error sending screenshot {i}: {e}", exc_info=True)

        await callback.answer("✅ Информация отправлена!")

    except Exception as e:
        logger.error(f"❌ Error in what_i_get_handler: {e}", exc_info=True)
        await callback.answer("❌ Ошибка отправки информации", show_alert=True)
