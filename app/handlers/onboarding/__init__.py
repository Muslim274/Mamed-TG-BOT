...
class NotAdminFilter(BaseFilter):
    async def __call__(self, event: Message | CallbackQuery) -> bool:
        ...
        if user_is_admin and hasattr(event, "data") and event.data:
            if (event.data.startswith("admin_approve_getcourse:") or 
                event.data.startswith("admin_reject_getcourse:") or
                event.data.startswith("broadcast_")):  # Добавляем broadcast callbacks
