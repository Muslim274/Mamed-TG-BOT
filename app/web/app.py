...
from app.database.crud import ClickCRUD, SaleCRUD, ReferralHistoryCRUD
from app.database.models import Click
...
async def robokassa_result(request: Request):
    ...
    masked_password = settings.ROBOKASSA_TEST_PASSWORD_2[:3] + "***" if settings.ROBOKASSA_TEST_PASSWORD_2 else "***"
    logger.info(f"   Test Password2: {masked_password}")
    ...
    logger.info(f"   Signature string: {OutSum:.2f}:{InvId}:***")  # Маскируем пароль
...
async def test_robokassa_webhook(request: Request):
    ...
    logger.info(f"Test webhook: Method={request.method}, URL={request.url}")
